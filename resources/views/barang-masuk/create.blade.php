...
@section('title', 'Tambah Barang Masuk - LogistikPro')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Tambah Barang Masuk</h2>
            <p class="mt-1 text-gray-600">Tambahkan transaksi barang masuk ke gudang</p>
        </div>
        ...
        <form action="{{ route('barang-masuk.store') }}" method="POST" class="space-y-4">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                    ...
                    <input type="date" name="tanggal_masuk" value="{{ old('tanggal_masuk', now()->toDateString()) }}" class="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" required />
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="supplier" value="{{ old('supplier') }}" class="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" required />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700">Qty Masuk</label>
                    <input type="number" name="qty_masuk" value="{{ old('qty_masuk', 1) }}" min="1" class="mt-1 w-full rounded-lg border border-gray-300 px-4 py-2 text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100" required />
                </div>
